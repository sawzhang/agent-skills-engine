...
class PackageManager:
    ...
    Package sources:
    - User scope: ``~/.agent-skills/packages/``
    - Project scope: ``.agent-skills/packages/``
    - ``pyproject.toml`` ``[tool.agent-skills]`` section
    """
    ...
    def __init__(
        ...
        self._user_dir = user_dir or (
            Path.home() / ".agent-skills" / "packages"
        )
        self._project_dir = project_dir or (
            Path.cwd() / ".agent-skills" / "packages"
        )
    ...
    def _resolve_pyproject(
        ...
        agent_skills = data.get("tool", {}).get("agent-skills", {})
        if not agent_skills:
            return None

        manifest = PackageManifest.from_dict(agent_skills)
    ...
    def load_manifest(self, path: Path) -> PackageManifest | None:
        ...
        Looks for:
        1. ``pyproject.toml`` with ``[tool.agent-skills]`` section
        ...
        if pyproject.is_file() and tomllib is not None:
            try:
                with open(pyproject, "rb") as f:
                    data = tomllib.load(f)
                agent_skills = (
                    data.get("tool", {}).get("agent-skills", {})
                )
                if agent_skills:
                    return PackageManifest.from_dict(agent_skills)
            except Exception:
                pass
