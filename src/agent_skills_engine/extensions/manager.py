...
from agent_skills_engine.events import EventBus
...
class ExtensionManager:
    ...
    1. Python entry points (group: agent_skills_engine.extensions)
    2. Global directory: ~/.agent-skills/extensions/*.py
    3. Project-local: ./.agent-skills/extensions/*.py
    ...
    def discover(self) -> list[tuple[str, str, Any]]:
        ...
        try:
            if sys.version_info >= (3, 12):
                ...
                eps = entry_points(group="agent_skills_engine.extensions")
            else:
                from importlib.metadata import entry_points

                all_eps = entry_points()
                if isinstance(all_eps, dict):
                    eps = all_eps.get("agent_skills_engine.extensions", [])
                else:
                    eps = all_eps.select(group="agent_skills_engine.extensions")
        ...
        # 2. Global directory
        global_dir = Path.home() / ".agent-skills" / "extensions"
        found.extend(self._discover_from_dir(global_dir, "global"))

        # 3. Project-local directory
        local_dir = Path.cwd() / ".agent-skills" / "extensions"
    ...
    def _import_from_path(self, name: str, path: Path) -> Any:
        ...
        module_name = f"agent_skills_ext_{name}"
