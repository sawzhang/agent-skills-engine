...
from skillkit.events import EventBus
...
class ExtensionManager:
    ...
    1. Python entry points (group: skillkit.extensions)
    2. Global directory: ~/.skillkit/extensions/*.py
    3. Project-local: ./.skillkit/extensions/*.py
    ...
    def discover(self) -> list[tuple[str, str, Any]]:
        ...
        try:
            if sys.version_info >= (3, 12):
                ...
                eps = entry_points(group="skillkit.extensions")
            else:
                from importlib.metadata import entry_points

                all_eps = entry_points()
                if isinstance(all_eps, dict):
                    eps = all_eps.get("skillkit.extensions", [])
                else:
                    eps = all_eps.select(group="skillkit.extensions")
    ...
    def _import_from_path(self, name: str, path: Path) -> Any:
        ...
        module_name = f"skillkit_ext_{name}"
