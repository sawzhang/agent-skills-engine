...
from skillkit.logging import setup_logging
...
def _config_show() -> None:
    ...
    config_paths = [
        ...
        Path.home() / ".config" / "skillkit" / "config.yaml",
    ]
...
def _config_path() -> None:
    ...
    paths = [
        ...
        ("User config", Path.home() / ".config" / "skillkit" / "config.yaml"),
    ]
...
async def cmd_chat(args: argparse.Namespace) -> None:
    ...
    if mode == "json":
        ...
        from skillkit.modes.rpc_mode import RpcMode

        rpc_mode = RpcMode()
        await rpc_mode.run(agent)
    else:
        # Interactive mode (default)
        from skillkit.modes.interactive import InteractiveMode
