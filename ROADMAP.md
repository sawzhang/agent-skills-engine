...
Agent Skills Engine improvement roadmap, based on architectural comparison with [pi-mono](https://github.com/badlogic/pi-mono) (a production-grade multi-provider agent SDK in TypeScript).
...
- `src/agent_skills_engine/events.py` — `EventBus`, event types, result types
- `src/agent_skills_engine/agent.py` — Integrate events into `AgentRunner` loop
...
**Files to create/modify**:
- `src/agent_skills_engine/models.py` — Add `StreamEvent` dataclass
- `src/agent_skills_engine/adapters/base.py` — Add `chat_stream_events()` to interface
- `src/agent_skills_engine/adapters/openai.py` — Map OpenAI stream chunks
- `src/agent_skills_engine/adapters/anthropic.py` — Map Anthropic stream events
...
**Files to create/modify**:
- `src/agent_skills_engine/model_registry.py` — `ModelDefinition`, `ModelRegistry`, `ModelCost`
- `src/agent_skills_engine/models_catalog.py` — Built-in model definitions (or `models_catalog.yaml`)
- `src/agent_skills_engine/adapters/base.py` — Add `TokenUsage` to `AgentResponse`
- `src/agent_skills_engine/config.py` — Accept `ModelDefinition` in `AgentConfig`
...
**Files to create/modify**:
- `src/agent_skills_engine/context.py` — `ContextTransformer`, compaction strategies, token estimation
- `src/agent_skills_engine/agent.py` — Integrate context pipeline into agent loop
- `src/agent_skills_engine/models.py` — Distinguish `AgentMessage` vs `LLMMessage`
...
**Files to modify**:
- `src/agent_skills_engine/runtime/base.py` — Add `on_output` parameter
- `src/agent_skills_engine/runtime/bash.py` — Stream subprocess output
- `src/agent_skills_engine/agent.py` — Emit `tool_execution_update` events
...
**Files to modify**:
- `src/agent_skills_engine/agent.py` — Add `abort()`, `steer()`, `follow_up()`, internal queues
- `src/agent_skills_engine/runtime/bash.py` — Respect abort signal in subprocess execution
- `src/agent_skills_engine/adapters/base.py` — Respect abort signal in LLM streaming
...
**Files to create/modify**:
- `src/agent_skills_engine/adapters/registry.py` — `AdapterRegistry`
- `src/agent_skills_engine/agent.py` — Use registry instead of single adapter
- `src/agent_skills_engine/extensions/api.py` — Expose `register_adapter()` to extensions
